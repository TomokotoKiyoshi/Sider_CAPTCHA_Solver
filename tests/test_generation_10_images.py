...
def run_test():
    ...
    output_dir = str(project_root / "test_output" / "captchas_10")
    max_images = 10
    workers = 4
    
    # 构建命令（使用相对于项目根目录的脚本路径）
    script_path = str(project_root / "scripts" / "data_generation" / "generate_captchas.py")
    cmd = [
        sys.executable,
        script_path,
        "--input-dir", input_dir,
    ...
    if annotations_path.exists():
        ...
        # 统计混淆策略分布
        confusion_counter = Counter(ann['confusion_type'] for ann in annotations)
        ...
        for confusion_type in ['none', 'rotation', 'perlin_noise', 'highlight', 
                               'confusing_gap', 'hollow_center', 'combined']:
            actual = confusion_counter.get(confusion_type, 0)
            expected_count = expected.get(confusion_type, 0) * max_images
            status = "✓" if actual == expected_count else "✗"
            print(f"  {confusion_type:15s}: {actual:4d} / {expected_count:4d} {status}")
        print("-" * 40)
        ...
        for ann in annotations:
            pos_key = f"({ann['bg_center'][0]},{ann['bg_center'][1]})"
            position_counter[pos_key] += 1
        
        print(f"\nPosition Distribution:")
        print(f"  Unique positions: {len(position_counter)}")
        print(f"  Theoretical positions: 48 (4x4x3)")
        print(f"  Position reuse: {'Yes' if len(annotations) > 48 else 'No'}")
